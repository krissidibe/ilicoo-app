...
> = {
  RIDE_REQUEST: { icon: "car-outline", color: "#6366f1", bgColor: "#6366f120", label: "Demande" },
  RIDE_ACCEPTED: { icon: "checkmark-circle-outline", color: "#059669", bgColor: "#05966920", label: "Accepté" },
  RIDE_REJECTED: { icon: "close-circle-outline", color: "#dc2626", bgColor: "#dc262620", label: "Refusé" },
  RIDE_CANCELLED: { icon: "alert-circle-outline", color: "#f59e0b", bgColor: "#f59e0b20", label: "Annulé" },
  RIDE_STARTED: { icon: "play-circle-outline", color: "#2563eb", bgColor: "#2563eb20", label: "Démarré" },
  RIDE_COMPLETED: { icon: "checkmark-done-outline", color: "#10b981", bgColor: "#10b98120", label: "Terminé" },
  RIDE_REMINDER: { icon: "time-outline", color: "#0ea5e9", bgColor: "#0ea5e920", label: "Rappel" },
  PAYMENT_RECEIVED: { icon: "cash-outline", color: "#10b981", bgColor: "#10b98120", label: "Paiement" },
  SYSTEM: { icon: "notifications-outline", color: "#64748b", bgColor: "#64748b20", label: "Système" },
};
    ...
    <Animated.View entering={FadeInRight.delay(index * 60).duration(350)}>
      ...
      >
        <View className="flex-row p-4">
          <View
            className="justify-center items-center rounded-2xl size-12 mr-3"
          ...
          <View className="flex-1">
            <View className="flex-row justify-between items-start mb-1">
              <View className="flex-row items-center flex-1 pr-2">
                <Text className="font-bold text-foreground text-[15px]" numberOfLines={1}>
                  {notif.title}
                </Text>
                {!notif.read && (
            ...
            <View className="flex-row items-center mt-2.5 gap-2">
              ...
              <View className="flex-row items-center">
                ...
                <Text className="text-[11px] text-muted-foreground ml-0.5">Détails</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        {notif.route && (
          <View className="flex-row items-center px-4 py-2.5 border-t border-gray-100 bg-gray-50/80">
            <Ionicons name="navigate-outline" size={12} color="#6366f1" />
            <Text className="text-[11px] text-muted-foreground ml-1.5 flex-1" numberOfLines={1}>
              {notif.route.pickupAddress}
            </Text>
            <Ionicons name="arrow-forward" size={10} color="#94a3b8" />
            <Text className="text-[11px] text-muted-foreground ml-1.5 flex-1" numberOfLines={1}>
              {notif.route.dropAddress}
            </Text>
          </View>
        )}
      </TouchableOpacity>
    ...
    open(
      <View className="px-5 pt-2 pb-7">
        <View className="flex-row items-center mb-4">
          <View
            className="justify-center items-center rounded-2xl size-14 mr-3"
            style={{ backgroundColor: cfg.bgColor }}
          >
            <Ionicons
              name={cfg.icon as keyof typeof Ionicons.glyphMap}
          ...
          <View className="flex-1">
            <Text className="text-lg font-bold text-foreground">{notif.title}</Text>
        ...
        {route ? (
          <View className="p-4 rounded-2xl border border-gray-200 bg-gray-50/80">
            <View className="flex-row items-center mb-3">
              <View className="p-2 rounded-full bg-blue-500/10 mr-2">
            ...
            <View className="flex-row items-center mb-3">
              <View className="p-2 rounded-full bg-rose-500/10 mr-2">
            ...
            <View className="flex-row items-center justify-between pt-3 border-t border-gray-200">
              ...
              <View className="flex-row items-center">
                ...
                <Text className="ml-1.5 text-sm font-bold text-foreground">
                  {route.distanceKm != null ? formatPrice(route.distanceKm, 1) : formatPriceDisplay(route.price)}
                </Text>
              </View>
            </View>
          </View>
        ...
        <View className="flex-row gap-3 mt-6">
          ...
          >
            <View className="flex-row items-center gap-2">
              ...
              <Text className="text-white font-semibold">Voir mes trajets</Text>
            </View>
          </Button>
          <Button
            variant="outline"
    ...
    <View className="flex-1 bg-gray-50">
      ...
      >
          ...
          <Animated.View entering={FadeInDown.duration(300)}>
            <View className="flex-row justify-between items-center mb-4">
              ...
              <TouchableOpacity
                ...
                className="py-2 px-4 rounded-xl bg-indigo-500/10"
              >
        ...
        {isLoading ? (
          ...
          >
            <View className="p-8 rounded-3xl bg-white mb-6" style={{ shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 3 }}>
              <MaterialCommunityIcons name="bell-sleep-outline" size={56} color="#c7d2fe" />
            </View>
